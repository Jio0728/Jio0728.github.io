...
사실 이 부분은 그냥 내가 끄적이고 싶어서 적는다. ViT는 얼굴 인식 task에도 적용 가능할까? 음, 좀 별로인 것 같기도 하지만 적용 가능하다.   
하지만 vit는 이미지들을 패치로 나누는데, 이 때 눈, 코, 입과 같이 얼굴을 인식하는데 중요한 부분들이 쪼개질 가능성이 있다. 또한 눈코입 간의 조합 또한 무시될 수 있다. 때문에 *Face Transformer For Recognition*이라는 논문에서는 이미지를 각 패치들이 서로 겹치게 자른다. 아직 논문 다 안 읽어서 여기까지만!ㅋㅋ
