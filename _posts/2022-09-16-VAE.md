...
<br>
VAE 논문에서는 reparametrization을 활용합니다.     
...
1. $\mathbb{L}\left(\theta, \phi ;x^{\left(i\right)}\right) = \mathbb{E}_{q_{\phi}\left(z|x\right)}\left [ -logq_{\phi}\left(z|x\right) + logp_{\theta}\left(x,z\right)\right ]$
2. $\mathbb{L}\left(\theta, \phi ;x^{\left(i\right)}\right) = -D_{KL}\left(q_{\phi}\left (z|x^{\left(i\right)} \right)||p_{\theta}\left (z \right)\right) +\mathbb{E}_{q_{\phi}\left(z|x^{\left(i\right)}\right)}\left [logp_{\theta}\left(x^{\left(i\right)}|z\right)\right ]$
<br>
