...
reparametrization이란, $q_{\phi}\left(z|x\right)$를 따르는 z를 $\widetilde{z} = g_{\phi}\left(\varepsilon , x\right)$ 로 변형해주는 것을 의미합니다. 이 때, $\varepsilon \sim p\left(\varepsilon \right)$입니다. 이러한 deterministic function을 이용하여 변형한 $\widetilde{z}$를 monte carlo gradient estimator에 넣으면 다음과 같은 식이 탄생합니다.     
<br>
