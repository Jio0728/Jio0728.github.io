...
<br>
이 논문에서는 이를 포함하여 true posterior density인 $p_{\theta}\left (z|x \right) = \frac {p_{\theta}\left (x|z \right) p_{\theta}\left (z \right) dz}{p_{\theta}\left (x \right)}$, 그리고 MLE와 MAP와 같은 [mean-field](https://en.wikipedia.org/wiki/Mean-field_theory) 방법을 사용하는데 필요한 모든 적분이 intractable한 상황을 가정합니다. 그리고 이런 상황을 **Intractability**라고 명명하고, Intractable한 사후확률을 추론하는 것을 목표합니다.
<br>
<br>
<br>
## 2-2. Variational Inference - VAE의 핵심 포인트
VAE는 **Variational Inference(VI)**.        
